...
def start_quiz(quiz_id):
    ...
    url = "https://www.owli.sk/studentLogin/{}".format(session_code)
...
def submit_question(session, question_index):
    ...
    question_type = questions_list[question_index - 1]['question_type']
...
# Check if the user has already answered this question
    previous_result = Result.query.filter_by(user_id=current_user.id, session_id=session_obj.id, question_index=question_index).first()
    if previous_result:
...
def show_results(session):
    try:
        ...
        return render_template('results.html', user_scores=sorted_user_scores,user_scores_list=user_scores_list, user =current_user, quiz=quiz, session=session)
    except Exception as e:
        # If there's an error, print the exception message
        print(str(e))
        return str(e), 500

@main.route('/quiz/<session>/export_results')
def export_results(session):
    try:
        ...
        df = pd.DataFrame(list(sorted_user_scores.items()), columns=['Nickname', 'Score'])
