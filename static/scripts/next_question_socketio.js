document.addEventListener('DOMContentLoaded', () => {
  ...
  let question_type = document.querySelector('input[name="question_type"]').value;
  ...
  let showBar = () => {
    ...
    if(question_type == 'true_false'){
      ...
      //setting the height of bars
      document.getElementById("bar-1").style.height += answer1Percentage+5 + "%";
      document.getElementById("bar-2").style.height += answer2Percentage+5 + "%";
...
if(question_type=='open_question'){
  ...
  let answerElements = {};
       ...
       // If an element for this answer already exists, remove it
        if(answerElements[answer]) {
            document.getElementById('answers').removeChild(answerElements[answer]);
        }

        const p = document.createElement('p');
        p.textContent = answer;

        // Increase the frequency count for this answer
        answerFrequency[answer] = (answerFrequency[answer] || 0) + 1;

        // Adjust the font size based on the frequency
        p.style.fontSize = `${answerFrequency[answer] * 10 + 10}px`;

        // Store the new element
        answerElements[answer] = p;
